...
if missing_deps:
    ...
    print("\nAlternatively, install all required packages with:")
    print("pip install numpy PyPDF2 faiss-cpu tqdm openai python-dotenv")
    sys.exit(1)
...
try:
    ...
    if OPENAI_API_KEY:
        ...
        print(
            "To set your API key, create a .env file with: OPENAI_API_KEY=your_key_here"
        )
        client = None
except ImportError:
    print("Warning: OpenAI package not installed. To use OpenAI embeddings:")
...
def process_pdf_directory(
    pdf_folder: str, recursive: bool = False, chunk_size: int = 1000, overlap: int = 200
...
def generate_answer(
    query: str, retrieved_context: List[Dict[str, Any]], model: str = "gpt-4o"
) -> str:
    """
...
def print_usage_example():
    """Print usage example that works with paths containing spaces."""
    print("\nUsage examples:")
    print("  With a simple path:")
    print('  python pdf.py --pdf_dir /simple/path --query "Your query here" --generate')
    print("\n  With a path containing spaces:")
    print(
        '  python pdf.py --pdf_dir "/path with spaces/to/pdfs" --query "Your query here" --generate'
    )
    print("\n  Note: Make sure to properly quote paths with spaces.")


def main():
    """Main function for the script."""
    parser = argparse.ArgumentParser(
        description="Extract, index, and search PDF documents"
    )
    parser.add_argument(
        "--pdf_dir", type=str, default=".", help="Directory containing PDF files"
    )
    parser.add_argument(
        "--output_dir", type=str, default="./index", help="Directory to save index"
    )
    parser.add_argument(
        "--recursive", action="store_true", help="Search for PDFs recursively"
    )
    parser.add_argument(
        "--chunk_size", type=int, default=1000, help="Size of text chunks in words"
    )
    parser.add_argument(
        "--overlap", type=int, default=200, help="Overlap between chunks in words"
    )
    ...
    parser.add_argument(
        "--generate", action="store_true", help="Generate answer using LLM"
    )
    parser.add_argument(
        "--help_examples", action="store_true", help="Show usage examples and exit"
    )

    if "--help_examples" in sys.argv:
        print_usage_example()
        return
    ...
    # Validate pdf_dir
    if args.pdf_dir != ".":
        args.pdf_dir = os.path.expanduser(args.pdf_dir)
